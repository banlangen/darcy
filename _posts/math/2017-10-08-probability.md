...
4. 对立事件，事件{A不发生}，这样的时间称为事件A的对立事件，记为$$\overline{A}$$(或$$A^c$$)。$$A \cap \overline{A} = \phi$$ 而 $$A \cup \overline{A} = \Omega$$。现在如果$$ \overline{A_1 \cup A_2 \cup \cdot \cdot \cdot \cup A_n} = \overline{A_1} \cap \overline{A_2} \cap \cdot \cdot \cdot \cap \overline{A_n}$$，那么对于集合B={A1...An中至少有k个事件发生}那么B的对立事件如何描述？$$\overline{B} = {A1...An中至多有k-1个事件发生}$$，这样处理有一个好处，至少事件的概率不好算，但是1个，0个这样的问题的概率就好算了，所以很多时候要将至少问题转换成至多的问题。有个公式叫做**对偶公式**$$ \overline{A_1 \cup A_2 \cup \cdot \cdot \cdot \cup A_n} = \overline{A_1} \cap \overline{A_2} \cap \cdot \cdot \cdot \cap \overline{A_n}$$ 和 $$ \overline{A_1 \cap A_2 \cap \cdot \cdot \cdot \cap A_n} = \overline{A_1} \cup \overline{A_2} \cup \cdot \cdot \cdot \cup \overline{A_n}$$
5. 事件的差，{A发生同时B不发生}这一事件称为A与B的差，记为 $$ A \minus B $$

#### 概率的定义与性质

##### 概率的定义
&emsp;&emsp;概率是随机事件发生大小的可能性的数字表征，即Prob是事件的函数
