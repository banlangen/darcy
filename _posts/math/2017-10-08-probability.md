...
3. 事件的积。A,B两个事件同时发生，事件{A,B同时发生}称为事件A与B的交集。$$A \cap B$$ 或者$$A \dot B$$ 或者AB。推广，事件{A1...An同时发生}称为事件A1...An的积，记为$$\cap_{i=1}^{n}{A_i}$$