...
>在前面问题的基础上，假设篮子中有两个苹果（可以认为这两个苹果完全一样），其他水果个数保持不变，还是从中挑选两样水果，请问有多少种挑选的方法？

&emsp;&emsp;这个问题用组合可就没有那么容易解答了，相对的，如果用生成函数，解答就会更容易理解和直观，对于苹果而言我们有三种选择方式，用多项式表达就是$$(x^0 + x^1 + x^2)$$，而其他的水果的选择方式不变，可以用二项式表达$$(x^0 + x^1)$$
#### Comments
